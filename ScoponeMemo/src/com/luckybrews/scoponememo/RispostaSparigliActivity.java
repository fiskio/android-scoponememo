...
public class RispostaSparigliActivity extends Activity implements OnClickListener {
	
	Context mContext = this;
	static final String TAG = "RispostaSparigliActivity";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        aux.setText("Seleziona le carte sparigliate");
    	...
    	switch(view.getId()){  
    		...
    		case R.id.buttonBar:  
    			...
    			if (!finito) {
    				...
    				if (!pass) {
    					risInt = new Intent(mContext, ScoponeMemoActivity.class);
    					risInt.putExtra("livello", ScoponeMemoActivity.livello);
    				} else {

    					if (!MainActivity.testaCoppie && !MainActivity.testaTutto) {
    						long liv = aumentaLivello();
    						risInt = new Intent(mContext, ScoponeMemoActivity.class);
    						risInt.putExtra("livello", liv);
    					} else {
    						risInt = new Intent(mContext, RispostaCoppiaActivity.class);
    					}
    				}
    				finito = false;
    				selected = new ArrayList<Integer>();
    				durata = -1;
    				pass = true;
        			RispostaSparigliActivity.this.finish();
    ...
    public void evidenziaRisposta() {
    	...
    	// size
    	if (ScoponeMemoActivity.dispari.size() != selected.size()) {
    		pass = false;
    	}
    	
    	// check
    	for (Integer i : selected) {
    		if (!ScoponeMemoActivity.dispari.contains(i)) {
    			// make red
    			setBackColor(i.intValue(), Color.RED);
    			pass = false;
    		} 
    	}
    	for (Integer i : ScoponeMemoActivity.dispari) {		
    		// make green
    	...
    	if (pass) {	
    		...
    		if (!MainActivity.testaCoppie && !MainActivity.testaTutto) {
    			...
    			tw.setText("Seleziona le carte prese a coppia");
    		}
    	} else {
    ...
    public void getTempo() {
    	...
    	if (MainActivity.coppieEnabled) {
    		record = settings.getFloat("RCD"+(ScoponeMemoActivity.livello+1), 0);
    	} else {
    		record = settings.getFloat("RSD"+(ScoponeMemoActivity.livello+1), 0);
    	}
    	// check record   	
    	if (durata < record || record == 0) {
    		SharedPreferences.Editor editor = settings.edit();
    		if (MainActivity.coppieEnabled) {
    			editor.putFloat("RCD"+(ScoponeMemoActivity.livello+1), durata);
        	} else {
        		editor.putFloat("RSD"+(ScoponeMemoActivity.livello+1), durata);
        	}
    	...
    	.setPositiveButton("Continua", new DialogInterface.OnClickListener() {
    		public void onClick(DialogInterface dialog, int id) {
    			...
    			if (!MainActivity.testaCoppie) {
    				...
    				Intent risInt = new Intent(mContext, RispostaCoppiaActivity.class);
    				RispostaSparigliActivity.this.finish();
    				startActivity(risInt);
    			}
    ...
    public void makeMessageNO() {
    	...
    	Collections.sort(ScoponeMemoActivity.dispari);
    	
    	AlertDialog.Builder builder = new AlertDialog.Builder(this);
    	builder.setMessage("Risposta corretta:\n" + ScoponeMemoActivity.dispari.toString())
    	.setTitle("ERRORE!")
    	.setCancelable(false)
    ...
    public long aumentaLivello() {
    	...
    	if (MainActivity.testaCoppie || MainActivity.testaTutto) {
    		Log.e(TAG, "AumentaLivello Sparigli");
    		return -1;
    	}
    	...
    	if (MainActivity.coppieEnabled) {
    		...
    		SharedPreferences.Editor editor = settings.edit();
    		editor.putLong("MAX_LIVELLO_CD", ScoponeMemoActivity.livello+1);
    		...
    		SharedPreferences settings = getSharedPreferences(MainActivity.PREFS_NAME, 0);
    		SharedPreferences.Editor editor = settings.edit();
    		editor.putLong("MAX_LIVELLO_SD", ScoponeMemoActivity.livello+1);
    		Log.d(TAG, "Sbloccato il livello " + (ScoponeMemoActivity.livello+1));
    		// Commit the edits!
    		editor.commit();
    		return ScoponeMemoActivity.livello;
