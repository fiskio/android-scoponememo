...
public class RispostaCoppiaActivity extends Activity implements OnClickListener {
	
	Context mContext = this;
	static final String TAG = "RispostaCoppiaActivity";
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        aux.setText("Seleziona le carte prese a coppia");
    	...
    	switch(view.getId()){  
    		...
    		case R.id.buttonBar:  
    			...
    			if (!finito) {
    				...
    				Intent risInt;
    				if (!MainActivity.testaTutto && pass) {
    					long liv = aumentaLivello();
        				risInt = new Intent(mContext, ScoponeMemoActivity.class);
        				risInt.putExtra("livello", liv);
    				} else {
    					risInt = new Intent(mContext, RispostaFiniteActivity.class);
    				}
    				finito = false;
    				selected = new ArrayList<Integer>();
    				durata = -1;
    				pass = true;
        			RispostaCoppiaActivity.this.finish();
    ...
    public void evidenziaRisposta() {
    	...
    	// size
    	if (ScoponeMemoActivity.coppie.size() != selected.size()) {
    		pass = false;
    	}
    	
    	// check
    	for (Integer i : selected) {
    		if (!ScoponeMemoActivity.coppie.contains(i)) {
    			// make red
    			setBackColor(i.intValue(), Color.RED);
    			pass = false;
    		} 
    	}
    	for (Integer i : ScoponeMemoActivity.coppie) {		
    		// make green
    ...
    public void getTempo() {
    	...
    	if (MainActivity.coppieEnabled) {
    		record = settings.getFloat("RCP"+(ScoponeMemoActivity.livello+1), 0);
    	} else {
    		record = settings.getFloat("RSP"+(ScoponeMemoActivity.livello+1), 0);
    	}
    	// check record   	
    	if (durata < record || record == 0) {
    		SharedPreferences.Editor editor = settings.edit();
    		if (MainActivity.coppieEnabled) {
    			editor.putFloat("RCP"+(ScoponeMemoActivity.livello+1), durata);
        	} else {
        		editor.putFloat("RSP"+(ScoponeMemoActivity.livello+1), durata);
        	}
    ...
    public void makeMessageNO() {
    	...
    	Collections.sort(ScoponeMemoActivity.coppie);
    	
    	AlertDialog.Builder builder = new AlertDialog.Builder(this);
    	builder.setMessage("Risposta corretta:\n" + ScoponeMemoActivity.coppie.toString())
    	.setTitle("ERRORE!")
    	.setCancelable(false)
    ...
    public long aumentaLivello() {
    	...
    	if (MainActivity.testaTutto) {
    		Log.e(TAG, "AumentaLivello Coppie!");
    		return -1;
    	}
    	...
    	if (MainActivity.coppieEnabled) {
    		...
    		SharedPreferences.Editor editor = settings.edit();
    		editor.putLong("MAX_LIVELLO_CP", ScoponeMemoActivity.livello+1);
    		...
    		SharedPreferences settings = getSharedPreferences(MainActivity.PREFS_NAME, 0);
    		SharedPreferences.Editor editor = settings.edit();
    		editor.putLong("MAX_LIVELLO_SP", ScoponeMemoActivity.livello+1);
    		Log.d(TAG, "Sbloccato il livello " + (ScoponeMemoActivity.livello+1));
    		// Commit the edits!
    		editor.commit();
    		return ScoponeMemoActivity.livello;
